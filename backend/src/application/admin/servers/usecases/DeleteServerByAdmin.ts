...
import { IDeleteServerByAdminUsecase } from "../interfaces/IDeleteServerByAdminUsecase";

@injectable()
export class DeleteServerByAdmin implements IDeleteServerByAdminUsecase {
  constructor(
  ...
  async execute(serverId: string): Promise<void> {
    ...
    await this._serverRepo.deleteServer(serverId);
  }
}