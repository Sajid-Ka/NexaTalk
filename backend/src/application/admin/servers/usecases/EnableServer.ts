...
import { IEnableServerUsecase } from "../interfaces/IEnableServerUsecase";

@injectable()
export class EnableServer implements IEnableServerUsecase {
  constructor(
  ...
  async execute(serverId: string): Promise<void> {
    ...
    await this._serverRepo.enableServer(serverId);
  }
}