...
import type { MessagePage, MessageItem } from "../types/message.types";
...
export const updateGroupMemberRoleApi = async ({
  ...
  api.patch<ApiResponse<GroupConversation>>(
    `/messages/groups/${conversationId}/members/${userId}/role`,
    { role },
  );
...
export const transferGroupOwnershipApi = async ({
  ...
  api.post<ApiResponse<GroupConversation>>(`/messages/groups/${conversationId}/transfer-owner`, {
    newOwnerId,
  });
...
export const addGroupMembersApi = async ({
  ...
  api.post<ApiResponse<GroupConversation>>(`/messages/groups/${conversationId}/members`, {
    participantIds,
  });
...
export const renameGroupApi = async ({
  ...
  conversationId: string;
  name: string;
}) => api.patch<ApiResponse<GroupConversation>>(`/messages/groups/${conversationId}`, { name });

export const deleteGroupApi = async (conversationId: string) =>
  api.delete<ApiResponse<null>>(`/messages/groups/${conversationId}`);

//channel apis
export const getChannelConversationApi = async ({
  serverId,
  channelId,
}: {
  serverId: string;
  ...
  >(`/messages/servers/${serverId}/channels/${channelId}/conversation`);
...
export const getConversationMessagesApi = async (
  ...
  api.get<ApiResponse<MessagePage>>(`/messages/conversations/${conversationId}/messages`, {
    params: { limit, cursor },
  });
