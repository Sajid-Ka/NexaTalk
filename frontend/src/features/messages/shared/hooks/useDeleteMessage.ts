...
export function useDeleteMessage() {
  ...
  return useMutation({
    ...
    onSuccess: async ({ data }, variables) => {
      ...
      const conversationId = deletedMessage.conversationId || variables.conversationId;

      queryClient.setQueryData(
        [ConversationQuery.CONVERSATION, conversationId],
      ...
      await Promise.all([
        queryClient.invalidateQueries({
          queryKey: [ConversationQuery.CONVERSATION, conversationId],
          refetchType: "active",
        }),
        queryClient.invalidateQueries({
